...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/48add42a1def5a90518762fd45d45e42.html";
...
var webviewIdentifier = 'viewfinder.webview';
function onRun(context) {
  ...
  webContents.on('did-finish-load', function () {}); // add a handler for a call from web content's javascript

  webContents.on('add-view-port', function (s) {
    addViewPortWithName(s);
  });
  webContents.on('externalLinkClicked', function (s) {
    NSWorkspace.sharedWorkspace().openURL(NSURL.URLWithString(s));
  }); // add a handler for a call from web content's javascript
  // webContents.on('add-view-port', s => {
  //   UI.message(s)
  //   webContents
  ...
  webContents.on('set-view-port', function (s) {
    setViewportByName(s);
  });
  webContents.on('remove-view-port', function (s) {
    removeViewportWithName(s);
  });
  browserWindow.loadURL(__webpack_require__(/*! ../resources/webview.html */ "./resources/webview.html"));
  var viewportsList = JSON.stringify(Object.keys(getPluginData()));
  console.log(getPluginData());
  webContents.executeJavaScript("updateListfromSketch(".concat(viewportsList, ")")).catch(console.error);

  function addViewPortWithName(name) {
    var viewports = getPluginData();
    var currentViewport = getCurrentViewport();
    viewports[name] = currentViewport;
    updatePluginData(viewports);
  }

  function removeViewportWithName(name) {
    var viewports = getPluginData();
  ...
  function getCurrentViewport() {
    ...
    viewport["center"] = {};
    viewport["center"]["x"] = parseFloat(canvasView.scrollOrigin().x);
    viewport["center"]["y"] = parseFloat(canvasView.scrollOrigin().y);
    viewport["zoom"] = parseFloat(canvasView.zoomValue());
    viewport["currentPageID"] = String(canvasView.currentPage().objectID());
  ...
  function setViewport(viewport) {
    var canvasView = context.document.contentDrawView();
    context.document.setCurrentPage(getPageByID(viewport["currentPageID"]));
    canvasView.zoomValue = viewport.zoom;
    canvasView.scrollOrigin = CGPointMake(viewport.center.x, viewport.center.y);
  }

  function getPageByID(id) {
    var canvasView = context.document.contentDrawView();
    var pages = canvasView.pages();

    for (var i = 0; i < pages.length; i++) {
      if (pages[i].objectID() == id) {
        return pages[i];
        break;
      }
    }
  ...
  function updatePluginData(viewports) {
    context.command.setValue_forKey_onDocument(JSON.stringify(viewports), 'viewports', context.document.documentData());
    viewportsList = JSON.stringify(Object.keys(getPluginData()));
    console.log(viewportsList);
    webContents.executeJavaScript("updateListfromSketch(".concat(viewportsList, ")")).catch(console.error);
  }

  function getPluginData() {
    var pluginData = context.command.valueForKey_onDocument('viewports', context.document.documentData());
  ...
  function setViewportByName(name) {
    var viewports = getPluginData();
    console.log(viewports);
    console.log(viewports[name]);
